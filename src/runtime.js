...
export const startProcess = async (account) => {
    ...
    try {
        const process = exec(`"${fetchmailPath}" ${args.join(' ')} --fetchmailrc "${getConfigPath(account.id)}"`, {
            env: {
                HOME: fetchmailHome,
        ...
        process.on('close', (code) => {
            console.debug(`[${account.id}] Process for account ${account.id} ended with code ${code}`);
            console.debug(`[${account.id}] stdout:`, stdout);
            console.debug(`[${account.id}] stderr:`, stderr);

            writeRuntimeInfo(account.id, new Date().toISOString(), stdout + stderr, code <= 1);
        });
    } catch (err) {
